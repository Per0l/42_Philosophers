...
if __name__ == '__main__':
	if not os.path.exists("philo/philo") or input("Recompile? (y/n)").lower() == 'y':
		os.system("make re -C philo/")
	if not os.path.exists("philo/philo"):
		print("Error: philo/philo binary not found")
	
	if not os.path.exists("tests_checker/"):
		os.mkdir("tests_checker")
	#Test 1
	print("Test 1: The philosopher should not eat and should die!")
	res = system_r("philo/philo 1 800 200 200").decode()
	...
	print("Test 4: ", end='')
	res = system_t("philo/philo 4 410 200 200  > tests_checker/test_4", s_check)

	#Test 5
	print("Test 5: A philosopher should die")
	res = system_r("philo/philo 4 310 200 100").decode()
